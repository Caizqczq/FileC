...
/// 阿里云百炼AI服务实现
/// </summary>
public class AlibabaCloudAiService : IAiAssistantService
{
    private readonly HttpClient _httpClient;
    private readonly AlibabaCloudAiOptions _options;
    private readonly ILogger<AlibabaCloudAiService> _logger;

    public AlibabaCloudAiService(
        HttpClient httpClient,
        IOptions<AlibabaCloudAiOptions> options,
        ILogger<AlibabaCloudAiService> logger)
    {
        ...
        // 注意：Content-Type不能在这里设置，需要在每次请求时设置
    }
    ...
    public async Task<AiAnalysisResponse> AnalyzeDocumentAsync(string content, string fileName, string contentType)
    {
        try
        {
            ...
            var response = await CallDashScopeAsync(prompt);
    ...
    /// <summary>
    /// 调用阿里云百炼DashScope API
    /// </summary>
    private async Task<string> CallDashScopeAsync(string prompt)
    {
        try
        {
            var request = new DashScopeRequest
            {
                ...
                {
                    Messages = new List<DashScopeMessage>
                    {
                        new DashScopeMessage
                        {
                            Role = "system",
                            Content = "你是一个专业的文档分析助手，能够准确分析和分类各种文档内容。请始终使用中文回复。"
                        },
                        new DashScopeMessage
                        {
                            Role = "user",
            ...
            var httpResponse = await _httpClient.PostAsync(_options.Endpoint, httpContent);

            if (httpResponse.IsSuccessStatusCode)
            {
                var responseContent = await httpResponse.Content.ReadAsStringAsync();
                var dashScopeResponse = JsonSerializer.Deserialize<DashScopeResponse>(responseContent);

                if (dashScopeResponse?.Output != null)
                {
                    // 优先从choices中获取内容
                    if (dashScopeResponse.Output.Choices?.Count > 0 && 
                        dashScopeResponse.Output.Choices[0].Message != null)
                    {
                        return dashScopeResponse.Output.Choices[0].Message.Content;
                    }
                    // 否则从text字段获取
                    else if (!string.IsNullOrEmpty(dashScopeResponse.Output.Text))
                    {
                        return dashScopeResponse.Output.Text;
                    }
                }

                _logger.LogWarning("DashScope API响应格式异常: {Response}", responseContent);
                return "AI服务响应异常";
            }
            else
            {
                var errorContent = await httpResponse.Content.ReadAsStringAsync();
                _logger.LogError("DashScope API调用失败: {StatusCode}, {Error}", 
        ...
        {
            _logger.LogError(ex, "调用阿里云百炼API失败");
            return "AI服务异常";
        }
    }
